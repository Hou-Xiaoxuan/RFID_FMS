...
public class TagReportListenerImplementation implements TagReportListener {
    ...
    public void onTagReported(ImpinjReader reader, TagReport report) {
        ...
        try {
            for (Tag t : tags) {
                /*
                 * String message="EPC: " + t.getEpc().toString() + "  LastTime: " +
                 * t.getLastSeenTime().ToString() + "  Rssi: " + t.getPeakRssiInDbm() +
                 * "  Phase: " + t.getPhaseAngleInRadians() + "  Channel:"+t.getChannelInMhz()
                 * "  AntennaPortNUmber:"t.getAntennaPortNumber()
                 */

                String message = t.getEpc().toString() + "#" + t.getLastSeenTime().ToString() + "#"
                        + t.getPeakRssiInDbm() + "#" + t.getPhaseAngleInRadians() + '#' + t.getAntennaPortNumber();
                System.out.println(t.getEpc().toString());

                // print the length of a PEC information
                // System.out.println(message.getBytes().length);

                GetInfomationOfRssiAndPhase.os.write(message.getBytes());

                // write message to *.txt
                GetInfomationOfRssiAndPhase.buffer.write((message + "\n").getBytes());
            }
